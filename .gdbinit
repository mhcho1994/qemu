...
run --plugin ./tests/tcg/plugins/libvirtual.so,logger=ws/log_config.txt,virtual=ws/virtuals.txt,detour=ws/detours.txt,modifier=ws/modifiers.txt -d in_asm,op -D qemu.log -machine mps2-an385 -monitor telnet:127.0.0.1:5555,server,nowait -semihosting --semihosting-config enable=on,target=native -kernel ws/RTOSDemo.axf -serial stdio -nographic -gdb tcp::1235 -S
